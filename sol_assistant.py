...
import queue
import time
# import logging # Keep commented unless deep debugging is needed
...
DEVICE = "AUTO" # Using AUTO device selection
MAX_NEW_TOKENS = 250
DO_SAMPLE = True
TEMPERATURE = 0.5
TOP_P = 0.9
...
# --- STT Configuration ---
WHISPER_MODEL_NAME = "openai/whisper-base.en" # or tiny for faster, less accurate
WHISPER_OV_MODEL_PATH = Path("./whisper_base_en_openvino")
AUDIO_SAMPLE_RATE = 16000
AUDIO_CHANNELS = 1
AUDIO_RECORD_DURATION_SECONDS = 5

# --- Model Loading and Conversion Logic for Phi-3 ---
def load_or_convert_phi3_openvino(status_callback):
    tokenizer = None
    ov_model = None
    try:
        status_callback(f"Phi-3: Attempting to load tokenizer for {PHI3_MODEL_NAME}...")
        tokenizer = AutoTokenizer.from_pretrained(PHI3_MODEL_NAME, trust_remote_code=True)
        status_callback(f"Phi-3: Tokenizer for {PHI3_MODEL_NAME} loaded successfully.")
    ...
    if ov_model_files_exist and tokenizer_files_exist_in_ov_path:
        status_callback(f"Phi-3: Found existing OpenVINO model and tokenizer files at {PHI3_OV_MODEL_PATH}. Loading...")
        try:
            ...
            status_callback("Phi-3: OpenVINO model and tokenizer loaded successfully from local path.")
            return ov_model, tokenizer
        except Exception as e:
            status_callback(f"Phi-3: Error loading existing OpenVINO model from {PHI3_OV_MODEL_PATH}: {e}. Will attempt re-conversion.")
            traceback.print_exc()
    else:
        if not PHI3_OV_MODEL_PATH.is_dir():
            status_callback(f"Phi-3: Optimized model directory {PHI3_OV_MODEL_PATH} not found.")
        elif not ov_model_files_exist:
            status_callback(f"Phi-3: OpenVINO model files (xml, bin, config.json) incomplete in {PHI3_OV_MODEL_PATH}.")
            for f_name in required_phi3_model_files:
                if not (PHI3_OV_MODEL_PATH / f_name).exists(): status_callback(f"Phi-3: Missing model file: {f_name}")
        elif not tokenizer_files_exist_in_ov_path:
            status_callback(f"Phi-3: Tokenizer files incomplete in {PHI3_OV_MODEL_PATH}.")
            for f_name in required_tokenizer_files:
                if not (PHI3_OV_MODEL_PATH / f_name).exists(): status_callback(f"Phi-3: Missing tokenizer file: {f_name}")
        status_callback("Phi-3: Will download and convert the model.")

    status_callback(f"Phi-3: Starting download and conversion for {PHI3_MODEL_NAME}...")
    try:
        status_callback(f"Phi-3: Exporting {PHI3_MODEL_NAME} to OpenVINO format...")
        ov_model_exported = OVModelForCausalLM.from_pretrained(
            PHI3_MODEL_NAME, export=True, trust_remote_code=True, device=DEVICE,
        )
        status_callback(f"Phi-3: Model exported to OpenVINO format.")
        status_callback(f"Phi-3: Saving OpenVINO model and tokenizer to {PHI3_OV_MODEL_PATH}...")
        ...
        status_callback("Phi-3: Reloading the saved OpenVINO model for use...")
        ...
        status_callback(f"Phi-3 ERROR during download/conversion: {e}")
        traceback.print_exc()
        return None, None

# --- Model Loading and Conversion Logic for Whisper STT ---
def load_or_convert_whisper_openvino(status_callback):
    processor = None
    ov_stt_model = None
    try:
        status_callback(f"Whisper: Attempting to load processor for {WHISPER_MODEL_NAME}...")
        processor = AutoProcessor.from_pretrained(WHISPER_MODEL_NAME, trust_remote_code=True)
        status_callback(f"Whisper: Processor for {WHISPER_MODEL_NAME} loaded successfully.")
    except Exception as e:
        status_callback(f"Whisper CRITICAL: Failed to load processor: {e}")
        traceback.print_exc()
        return None, None
    ...
    if ov_model_files_exist and processor_files_exist_in_ov_path:
        status_callback(f"Whisper: Found existing OpenVINO model and processor files at {WHISPER_OV_MODEL_PATH}. Loading...")
        try:
            ...
            status_callback("Whisper: OpenVINO STT model and processor loaded successfully from local path.")
            return ov_stt_model, processor
        except Exception as e:
            status_callback(f"Whisper: Error loading existing OpenVINO STT model/processor: {e}. Will attempt re-conversion.")
            traceback.print_exc()
    else:
        if not WHISPER_OV_MODEL_PATH.is_dir(): status_callback(f"Whisper: Optimized model directory {WHISPER_OV_MODEL_PATH} not found.")
        elif not ov_model_files_exist:
        ...
        status_callback("Whisper: Will download and convert the STT model.")

    status_callback(f"Whisper: Starting download and conversion for {WHISPER_MODEL_NAME}...")
    try:
        status_callback(f"Whisper: Exporting {WHISPER_MODEL_NAME} to OpenVINO format...")
        ...
        status_callback(f"Whisper: Model exported to OpenVINO format.")
        status_callback(f"Whisper: Saving OpenVINO model and processor to {WHISPER_OV_MODEL_PATH}...")
        ...
        status_callback(f"Whisper: OpenVINO STT model and processor saved.")
        status_callback("Whisper: Reloading the saved OpenVINO model and processor for use...")
        ov_stt_model = OVModelForSpeechSeq2Seq.from_pretrained(WHISPER_OV_MODEL_PATH, device=DEVICE, trust_remote_code=True)
        processor = AutoProcessor.from_pretrained(WHISPER_OV_MODEL_PATH, trust_remote_code=True)
        status_callback("Whisper: OpenVINO STT model and processor ready for use.")
        return ov_stt_model, processor
    except Exception as e:
        status_callback(f"Whisper ERROR during download/conversion: {e}")
        traceback.print_exc()
        return None, None

# --- Phi-3 Response Generation ---
def generate_phi3_response(model, tokenizer, prompt_text, conversation_history, pdf_content=None, pdf_name=""):
    ...
    pdf_context_prompt = ""
    if pdf_content:
        truncated_pdf_content = pdf_content[:1500] # Adjusted PDF context length
        pdf_context_prompt = (
    ...
    if pdf_content: # Limit history when PDF is active
    ...
    #     print(f"WARNING: Prompt was truncated to {max_prompt_len} tokens.")
    ...
    try:
        ...
        print(f"Error during Phi-3 model.generate: {e}")
        traceback.print_exc()
...
def extract_text_from_pdf_static(pdf_path):
    try:
        ...
        if text:
            return text, pdf_name, f"Successfully loaded '{pdf_name}'. Ready for questions."
        else:
            return "", pdf_name, f"Loaded '{pdf_name}', but no text could be extracted or PDF is empty."
...
class ChatApp(ctk.CTk):
    ...
    def finalize_model_loading(self):
        ...
        if all_core_ready: self._update_status("Sol is ready to help!")
            ...
            self._update_status("Sol can think, but voice input has an issue.")
            current_placeholder = "Ask Sol (text only)..."
        elif whisper_ready: self._update_status("Sol can hear you, but the thinking part has an issue.")
    ...
    def _process_pdf_background(self, filepath):
        ...
        def update_gui_with_pdf_data():
            ...
            self.pdf_status_text_var.set(result_message) # Show full message from extraction
            if self.current_pdf_text:
                ...
                self.clear_pdf_button.configure(state="disabled")
                self.user_input.configure(placeholder_text="Ask Sol...") # Revert placeholder if PDF load failed
    ...
    def _add_message_to_display(self, sender, message):
        ...
        display_name = sender
        if sender == "Sol-Internal": display_name = "Sol" # Internal AI name
    ...
    def _display_phi3_response_and_update_history(self, response_text, user_prompt, is_error=False):
        self._add_message_to_display("Sol", response_text) # Standardize AI display name
        if not is_error:
